#!/usr/bin/env node
/**
 * assets/village2/students/<두 자리 번호>/original 폴더의 이미지(PNG/JPG/JPEG/GIF)를 WebP로
 * 일괄 변환해 같은 학생의 assets/village2/students/<번호>/items 폴더에 저장하고, 그 결과를
 * 바탕으로 assets/village2/students/<번호>/items/items.json을 자동으로 만들거나 덮어쓴다.
 *
 * 두 번째 마을(village-2)의 학생(B01~B18) 전용 버전이다 - 폴더만 assets/students 대신
 * assets/village2/students를 보고, id 접두사만 s(S01~S18) 대신 b(B01~B18)를 쓴다는 점만
 * 다르고, 그 외 동작은 scripts/convert-students-assets.js와 완전히 같다
 * (assets/village2/README.md 참고).
 *
 * - 학생 번호(01~18)를 하나하나 하드코딩하지 않는다. assets/village2/students 폴더 밑에서
 *   이름이 두 자리 숫자인 폴더를 전부 찾아 그 목록을 그대로 순회한다.
 ...
 * - 기존 items.json이 있으면 먼저 읽어서, 같은 결과물 파일명을 가진 항목은 id를 그대로
 *   재사용한다 (Supabase에 저장된 item_id가 재변환한다고 바뀌지 않도록 - 첫 번째 마을과 동일).
 *
 * 실제 변환 로직(파일 하나 변환, items.json 항목 만들기)은 scripts/lib/convert-images.js에
 * 있다 (첫 번째 마을 스크립트들과 공유).
 *
 * 실행: npm run convert:village2:students
 */
...
const STUDENTS_DIR = path.join(ROOT_DIR, "assets", "village2", "students");

// assets/village2/students 밑에서 이름이 "01".."18"처럼 정확히 두 자리 숫자인 폴더만 학생
// 폴더로 인식한다. js/main.js의 getPersonalItemsPath()가 접속 코드(B01 등)에서 뽑아낸 번호를
...
async function convertStudent(studentNumber) {
  ...
  if (results.length > 0) {
    ...
    const entries = buildItemCatalogEntries(results, {
      idPrefix: `b${studentNumber}`,
      imageDirPath: `assets/village2/students/${studentNumber}/items`,
      previousEntries,
    });
...
async function main() {
  ...
  for (const studentNumber of studentNumbers) {
    try {
      ...
      console.error(`  [B${studentNumber}] 처리 중 예상치 못한 오류로 건너뜁니다:`, err.message);
      totalFailure += 1;
    }
  }
...
// `node scripts/convert-village2-students-assets.js`로 직접 실행했을 때만 바로 돈다.
